...
class DBProvider {
  ...
  addNoteInDB(Password newNote)async{
    final db =await database;
    if(newNote.userName.trim().isEmpty) newNote.userName = 'Untitled Note';
    int id = await db.transaction((transaction) {
      transaction.rawInsert(
          'INSERT into Notes(title,username,appname,password) VALUES ("${newNote.appName}", "${newNote.userName}", "${newNote.password}");');
    });
    newNote.id = id;
    print('Note added: ${newNote.appName} ${newNote.userName}  ${newNote.password}');
    return newNote;
